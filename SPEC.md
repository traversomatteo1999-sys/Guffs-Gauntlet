...
- **⏩ CURRENT FRONTIER (2026-07-10):** Phases 0–52 done. **Phase 51 (11 tasks) + Phase 52 (8 tasks) ✅ ALL BUILT, merged & pushed** — `sw` v88 @ origin/main. Each: branch-per-task → merge `--no-ff` → push, with a `tests/p5*-*.test.js` driver + syntax gate. HIGH/MED-HIGH tasks reviewed: P51.6 + P51.4 (4-lens Workflows, 2 + 6 fixed), P52.4 (single-agent, 2 fixed), P52.5 (single-agent, clean). `npm test` = 66/66 green. **The ONLY spec item still deferred repo-wide: P49.6 (Italian i18n).** **⬜ NEW — Phase 52 (2026-07-10 batch 2, 6 items) is SPECCED below but NOT built:** command-zone commander options · siege/battle popup · enemy-commander creature-card layout · legendary-planeswalker copy discard · **creature/permanent subtypes** (MED-HIGH) · wire the new soundtracks. **P52.1 reverses P50.6 (user-requested) — flag before building.**
...
| **Phase 42 — Item duration & economy coherence** | ✅ **DONE & verified** — passives REALLY last 1 descent (stash-loop root cause fixed) · Sapper's-Map row consumption · Scholar/Tome price-invert · heal/damage price curves (user anchor: Healing Draught 7g). 4 new jsdom drivers (t_p42_1–4) + a review-fix driver (t_p42_2b); syntax gate + id-diff (245/245) clean; 20/20 prior regression stable; 3-lens adversarial review → 1 LOW finding (legacy-save stale-map double-count) fixed in `migrate`. |
| &nbsp;&nbsp;P42.1 Passives really expire — `saveUnusedItems`/`applyStashItems` stash consumables only (+ drop dead `descents` field from all 6 write sites + migrate) | ✅ **DONE & verified** (the immortal-passive stash loop is broken; unused Ward/Aegis/Phoenix no longer return every descent; unused consumables still persist; `descents` has 0 readers/writers, only a `delete` in migrate) |
...
| &nbsp;&nbsp;P45.3 Animation/FX layer decoupled from the innerHTML-rebuild render (damage numbers · life flash · tap/move motion) | ✅ **DONE & verified** (body-level `#fx` overlay, pointer-events:none; `fxFloat`/`fxFlash`/`fxLife` spawn self-removing floating damage/heal numbers + life-bar flash at `adjLife`/combat/`dmgBoss`; spawn anim keyed to new permanents via `_bornIds` (`.crea.isnew`, cleared on new room) so tiles don't re-fade every render; tiered reduced-motion — decorative motion near-instant but `.fxnum`/`.barflash` still communicate. t_p45_3 25/25. *Card-travel ghosts deferred — not an AC.*) |
| &nbsp;&nbsp;P45.4 Sound design: wire the orphan `heal` cue + ~12 missing cues · volume slider · optional ambient pad (off by default) | ✅ **DONE & verified** (master GainNode + volume slider (DB.vol) + optional ambient pad (DB.pad, off); orphan `heal` wired via `adjLife`; 13 new quiet cues — purchase/loot/use/deploy/draw/phase/descend/wheel/wheelland/sting/enemycast/loyalty/counter; nothing fires on a pure render(); headless-safe. t_p45_4 45/45 oscillator-count) |
| &nbsp;&nbsp;P45.5 Moxfield-style ≥1100px desktop layout (multi-pane battlefield + side rail) over the same panels | ✅ **DONE & verified** (`buildTabs` wraps the 4 tabs + stack in a persistent `#battlefield`; a CSS `@media(min-width:1100px)` alone flips phone-stack↔3-rail grid — no resize listener: **left** Info (You/Foe+log, spans 2 rows) · **centre** Enemy board (row1) + Your board (row2) · **right** Action (row1) + Stack (row2); rails force-open + sticky heads + per-cell scroll; phone layout pixel-unchanged; header shows level+room. t_p45_5 29/29 + visual pass) |
| &nbsp;&nbsp;P45.6 Stack popup stops occluding the board (dockable) + log legibility rework | ✅ **DONE & verified** (≤1100px `.stackpop` is a bottom sheet ~44vh with pass-through above so the board stays visible/interactive, + a ⤢ expand-to-fullscreen toggle; ≥1100px it's the static right-rail cell (FAB hidden). *Log channel-icon/aria-live rework folded into P46.4 accessibility pass.*) |
| **Phase 46 — Completeness & hardening + FINAL CLEAN-UP & VALIDATION** | ✅ **DONE & validated (2026-07-06)** (specced 2026-07-04; ran LAST). Step 1: P46.1 checked-in `npm test` suite · P46.7 cleanup · P46.6 deploy files. Step 2: P46.2 error toast · P46.3 ⚙ Settings · P46.4 a11y · P46.5 onboarding. **P46.8 gate PASSED** — `npm test` 25/25 · clean id-diff · live-browser + live-Scryfall smoke · a 3-lens final review (2 confirmed cross-phase bugs fixed). **2 items handed to the user:** balance playtest + the one-time Netlify Import-from-Git connection (checklists in §P46.8). See the VALIDATION RECORD under `# PHASE 46`. |
| &nbsp;&nbsp;P46.1 Checked-in regression suite (`package.json` + `tests/` jsdom drivers + full-descent driver; `npm test`) | ✅ **DONE** (`package.json` (private, jsdom devDep) + `tests/` with `run.js` runner, `harness.js`, syntax gate, id-set guard (`ids.snapshot.txt`, 257 ids), SW-drift guard, full-descent + 15 system drivers + p46-cleanup; `.gitignore`; `tests/README.md`. **`npm test` → 19/19 green, deterministic** (3× re-run), offline. Flaky RNG enemy-run excluded by design.) |
| &nbsp;&nbsp;P46.2 Error reporting: `window.onerror`/`unhandledrejection` → player-visible toast + downloadable debug dump | ✅ **DONE** (Step 2) (`window.onerror`+`unhandledrejection` buffer (cap 20) + `sys`-log + non-blocking sticky `#toast` with a ⬇ debug-report JSON download `{game,version,errors[],stripped state}`; a render crash sets `_renderBroke` so `settleHistory`/`doAutosave` skip a half-mutated `S` until a clean render clears it. `tests/p46-2-errors` 17/17.) |
| &nbsp;&nbsp;P46.3 ⚙ Settings panel (volume · text size · animation level · mute; consolidates exports) | ✅ **DONE** (Step 2) (⚙ Settings `#overlay` from the menu + controls bar: volume+mute+ambience · text size S/M/L→root font · animation full/soft/off/auto (a JS-set `html.motion-*` class overrides the OS query both ways; off keeps static damage numbers) · consolidated export/import save+library + debug report; persisted on `DB.settings`, applied at boot via `loadSettings`. `tests/p46-3-settings` 22/22 + visual pass.) |
| &nbsp;&nbsp;P46.4 Accessibility: `aria-live` log · dialog focus management · contrast/touch audit | ✅ **DONE** (Step 2) (`#log` `aria-live=polite`+`aria-relevant=additions`; `installA11y` MutationObserver focus-trap (Tab cycles the topmost overlay, focus restored on close) + keydown: **Esc closes any open overlay EXCEPT the combat resolver** (Approve/Cancel-only kept); `aria-label` on the emoji-only creature ✕; contrast audited — `--bone-dim` on `--ink` **7.08:1** (AA), `--bone` **13.85:1** (AAA). `tests/p46-4-a11y` 14/14.) |
| &nbsp;&nbsp;P46.5 First-run onboarding (auto-offer the tutorial once per profile) | ✅ **DONE** (Step 2) (`blankProfile` `seenTutorial:false`; after the arrival cutscene, `offerTutorial` gives 📖 Learn / ▶ Just-play — flag set the moment it shows so decline/reload never re-prompts; `migrateProfiles` backfills existing profiles to seen; sandbox + continued runs never prompted; the ❓/menu opt-in entries stay. `tests/p46-5-onboarding` 16/16.) |
| &nbsp;&nbsp;P46.6 Deploy pipeline: `netlify.toml` + GitHub continuous deploy + SW-bump checklist (retire manual drag-drop) | 🟡 **FILES DONE** (`netlify.toml` (publish root, no build, SW/shell no-cache headers) + README rewrite (CD primary, drag-drop fallback, SW-bump checklist, `gg-cache-v49` fixed — it had drifted from v48) + `tests/sw-version.test.js` drift guard. **BLOCKED-ON-USER:** the one-time Netlify *Import-from-Git* connection (empty build cmd, publish `/`) — until you flip it, drag-drop stays the fallback; verified live at P46.8.6.) |
| &nbsp;&nbsp;P46.7 Code clean-up: dead `pwAct` body · legacy `S.pw` field · any 41–45 leftovers | ✅ **DONE** (deleted `pwAct()` (retired S.pw upkeep auto-fire, no callers); dropped `fresh()`'s legacy `pw:null`; `migrate()` still upgrades an old-save `S.pw`→real `S.enemyWalkers`; grep sweep clean — remaining hits are live fns / deliberate no-ops (`carryInvForward`→`[]`, inert `enemyMana` emblem handler) / retired-comments. `tests/p46-cleanup.test.js` 7/7 (incl. v40-save migrate); id-diff clean; regression green.) |
| &nbsp;&nbsp;P46.8 FINAL VALIDATION: full regression + live browser smoke + multi-agent adversarial review + balance playtest checklist + PWA/offline/deploy verification | ✅ **DONE (2026-07-06)** — all 6 steps in the VALIDATION RECORD (`# PHASE 46`): (1) `npm test` 25/25 offline+deterministic incl. both-modes/all-difficulties; (2) syntax + clean cumulative id-diff; (3) live Edge board@1360/phone@380 + live Scryfall import + offline-font guarantee + legacy-save migrate; (4) 3-lens final review → 2 confirmed cross-phase bugs fixed (Esc-menu phantom board · Merchant wrong-purse), 3 refuted; (5) balance-playtest checklist **handed to user**; (6) deploy files done, Netlify CD connection **blocked-on-user** (checklist handed over). |
| **Phase 47 — Real art pass** (per-warden icons/backdrops · menu · lore images · cutscene reveal) | ✅ **DONE & merged** (`141a5f8`/`468ccf0`) — 9 user-supplied `Pictures/*.jpg` wired in via `ENEMY_ICON`/`DUNGEON_BG`/`MENU_BG` (kept separate from the `ART` base64 lore frames): `#bossPortrait` uses the dedicated warden icon · `#bgart` repaints per-warden on `enterRoom` · `Menu.png` backdrop · real Ember/Conclave lore frames · `#cutscene` opaque per-scene backdrop + on-Continue board drop-in. **P47.1** compressed the 9 PNGs→JPEG (~19MB→~1.9MB, opaque RGB so lossless-of-alpha). `sw.js`→v51; 25/25 + 30-assert art drive. *(back-filled into the tracker 2026-07-07)* |
| **Phase 48 — Layout revert + onboarding order + Vael/Ash rename & siege move** | ✅ **DONE & merged** (`131e43c`/`6e438f1`) — **P48:** removed the P45.5/6 ≥1100px three-rail desktop grid; per §3b/P1.1 the layout is **four collapsible cornice tabs stacked VERTICALLY at every width** (Info · Action · Enemy Board · Your Board), each full-width, You/Foe + Turn-flow/Your-attack side-by-side inside their tab (fixes the tab-order complaint); the stack stays the P45.6 bottom-sheet popup; dropped the moot P47.2 duo override. First-run tutorial offer now fires **after all story cutscenes** (arrival→gate) and "Learn the ropes" reveals the board then opens the tutorial. `sw.js`→v53. **P48.3:** the enemy walker-commander is now **"Ash the Guardian"** (abilities already Ash-themed: Ember Whisper / Grasping Ash / Avatar of Ash); villain name stays *Vael, the Ember Tyrant*. Siege is data-driven via `room.siege`+`fieldBossSiege(cfg)`; **Vael no longer fields one — Grakk fields "Siege of the Ember Gate"** (in play, def 6, break deals 6, heals him +1/upkeep — was Vael's +2); retired `fieldVaelBattle`/`_vael`. `sw.js`→v55; 25/25. *(back-filled into the tracker 2026-07-07)* |
| **Phase 49 — Fix batch 2026-07-07** (enemy-box UI · spell/stack routing · counters on non-creatures · economy retune · **Italian i18n + flag** · campaign flow · enemy deck realism/removal · enemy coloured mana · combat/PW/siege UX · item persistence · Ash-the-Guardian ability rewrites) | 🟢 **10 of 12 DONE, merged & pushed (`origin/main` @ `2714681`)** — the user's 30-item fix list, grounded below in `play.html`. ✅ P49.1–.5 · .7 · .8 · .10 · .11 · .12 built (each: `tests/p49-*.test.js` + syntax gate; risky ones reviewed; `npm test` 35/35 green). **Reversals landed as specced: #20/21 automate enemy instants (P49.3, vs P4.2/4.4) · #22 PW abilities off-stack (P49.10, vs P38). #15 commander tax = real-MTG +2 per command-zone cast (P49.2).** **⬜ REMAINING (2 mega-tasks, deferred to their own weekly windows): P49.6 Italian i18n · P49.9 enemy coloured mana + London mulligan (vs P44).** See `# PHASE 49` build record. |
| &nbsp;&nbsp;P49.1 Enemy-box UI: collapsible/grouped mana-modifier sub-box + ⓘ · bigger warden icons · Library Cards/Tokens default-collapsed | ✅ done (bullets 1·5·4) |
...
| &nbsp;&nbsp;P49.3 Enemy instants fully automated (spend mana + card→graveyard, preserve counterability) + expandable enemy stack spells | ✅ done (bullets 20·21) — **reverses P4.2/4.4** |
...
| &nbsp;&nbsp;P49.6 Italian localization (cutscene/lore/UI/tutorial/info/abilities/shop/menus/library/settings, keep NAMES) + main-menu flag selector (default EN) | 🟢 **7a infra + 7b narrative + 7d/7e mode-screen chrome DONE (v89–v91) + all 12 INFO help bodies (v94) + combat resolver/attack (v95) + the FULL tutorial (v96)** — + campaign/sandbox/store/library screens translate via an `_trText` dictionary chokepoint at open time (level name/teaser wrapped at the render site). Earlier: — `DB.lang` (default en) · `t()`/`L()` dictionary helpers (graceful EN fallback) · 🌐 menu flag + Settings language row · `applyLang()` static-chrome tree-walker · `setLang()` save+reload. Translated: menu · settings · tabs · difficulty/saves · phase/turn-flow labels · INFO titles + short bodies · **the full narrative (arrival + all 3 wardens' intros/quotes/death-lines + victory vaelEscape/guffFreed/epilogue + defeat) + all cutscene titles/quotes/buttons** · shop rarity words. **EN unchanged, IT boots + full turn cycle 0 errors; proper nouns + MTG keywords stay EN.** Extensible dictionary — remaining prose (full tutorial, long info bodies, mode-screen dynamic text, item descriptions, ~305 log lines) falls back to EN, translatable by adding dictionary entries. |
| &nbsp;&nbsp;P49.7 Campaign flow: Level-1 "To be continued…" → Menu + replay-lock + cancel/abandon campaign | ✅ done (bullets 8·29) |
| &nbsp;&nbsp;P49.8 Enemy deck realism: colour-loyal permanent removal in every deck · deeper difficulty deck variation · Grakk gets ramp/ability permanents · balance | ✅ done (bullets 9·25·26) |
...
| &nbsp;&nbsp;P50.9 Smarter enemy strategy — mana spend + attack/block EV that minimizes self/creature loss | ✅ **done v67** — 3 gated (`enemyLuck()>=0`) heuristics: profitableBlock holds a bomb the player can GANG-kill favorably · aiBlocks won't throw a valuable body to chump a non-lethal hit when the boss is healthy · enemyHoldUpReserve develops instead of sandbagging when behind on board. Single-agent adversarial review = NO findings |
...
| &nbsp;&nbsp;P51.1 Card-creation: turn ANY permanent into a creature (with P/T), optionally **retaining** old types | ✅ **done v75** — 🐾 "also a creature" toggle for artifact/enchantment → `props.animate`; resolves into `S.my.creatures` with retained `types:[t,'creature']` + `animated`; tile shows "Artifact/Enchantment Creature" badge; no prowess trigger |
...
| &nbsp;&nbsp;P51.3 Enemy commander (creature **and** walker) gets full card-option parity (deal-damage etc.); fix "put to hand"/"copy" overlapping "slay" | ✅ **done v76** — one `.slay` corner button + inline `.mini` secondaries (`_cmdSecondaryBtns`); walker gets ⧉copy + Slay; `dealDmg('cmd'/'pcmd')` routes lethal (creature→removeRef, walker→loyalty/enemyWalkerDies); creature-cmd drawer gains `markerRow` (deal/deathtouch/copy/markers) |
| &nbsp;&nbsp;P51.4 **Partner commanders** — import `partner` from Scryfall; play a 2nd commander; show both boxes stacked on the "You" panel | ✅ **done v79** — `S.pcmd2` slot; ⚑ creator toggle + Scryfall Partner import; slot-aware lifecycle (`pcmdOf`/`setPcmdOf`, `_slot` on the object, cast/deploy/toHand/toZone/death/counter/fresh-dungeon all route by slot); `#pcmdBox2` + `renderOneCmd`; `getObj`/`getCre`/`dealDmg` handle pcmd2; fresh+migrate seed/backfill. **4-lens review → 6 confirmed fixed** (setCommander/setStartCommander made slot-aware so re-designating never clobbers the other slot / bypasses the Partner guard; freshGameForDungeon dual-slot collision; pcmd2 tax label) + 1 refuted (single cmd-damage counter is pre-existing) |
...
| &nbsp;&nbsp;P51.6 Enemy attack-target intelligence — threat order PW > creature > enchant > artifact; face vs walkers/sieges/battles | ✅ **done v78** — `THREAT_TIER` split (enchant −2 > artifact −3), `objKind` returns artifact/enchant, player perms `ctype`-tagged; `aiTargets` rewritten to spread across face+walkers+player-battles (gated: easy legacy / std single / brutal spread); `b:` plumbing in renderCombat/approveCombat/predictCombat. **4-lens review → 2 MEDIUM fixed** (predictCombat dropped battle-damage preview [v78]; aiTargets fall-through broke the "easy=legacy" invariant — now commits to the top object, `tried` counter not `placed` [v80]) |
...
| &nbsp;&nbsp;P52.4 Copying a **legendary planeswalker** → popup: pick which duplicate goes to the **graveyard**, or tick "**copy isn't legendary**" | ✅ **done v87** — legendary copy on your board opens `openLegendRule` (keep one → others via killMy honouring dies/exile; or "copy isn't legendary"); hooked in both `copyPermanent` AND `dupMy`; closeOverlay/Esc default = keep-both. Single-agent review → 1 MED (dupMy bypass) + 1 LOW (3+ dupes) fixed |
| &nbsp;&nbsp;P52.5 **Creature/permanent subtypes** on every card (enemy + player) — auto-migrate imported library, manual editor, walker type = its name, multi-type (e.g. Soldier Artifact), enchantment types (Aura/Elemental) | ✅ **done v88** — `subtypes[]` field; Scryfall import parses `type_line`; creator `castSubtypes` input; walker auto-derives its name; `migrate`+`migrateProfiles` backfill (SAVE_V 44→45); type-line display on player+enemy creature tiles; enemy commanders + spawn 6th-arg carry subtypes |
...
# PHASE 42 — Item duration & economy coherence ✅ DONE & verified (2026-07-05)
...
# PHASE 43 — Campaign & Sandbox: two game modes + multi-level infrastructure ✅ DONE & merged (2026-07-05)

**BUILT & VERIFIED (each sub-task: jsdom driver + syntax gate + id-diff; phase gate: full regression + a 4-lens adversarial-review workflow, all 8 confirmed findings fixed).** P43.1 per-mode buckets `p.camp`/`p.sand` + `migrateProfiles` (folds legacy flat gold/pending/stash into camp, routes a global `DB.save` to its owner's `camp.run` — fixes the cross-profile Continue bug) + `S.mode`/`levelIdx` + mode-keyed save slots (`getSave`/`setSave`) · P43.2 `LEVELS[]` registry (DUNGEON/`ROOM_ART` aliased via `let DUNGEON`/`_levelArt`, line 614 untouched), `setLevel`/`curLevel`/`battleId` · P43.3 menu mode-choice (⚔ Campaign / 🏟 Sandbox), campaign screen + sandbox grid, `startBattle({mode,levelIdx,roomIdx,diff})` collapsing restart/startNewDescent, bucket-keyed Merchant · P43.4 between-level super-advance (`descendToNextLevel` carries life/max/gold/consumables; `levelClearTransition`+wheel; dormant with one level → `win()`), forward-only · P43.5 sandbox single-battles (`endSandboxBattle` pays the sandbox purse + `p.sand.stats`, HP refresh, no walk/wheel; sandbox skips the campaign arrival intro) · P43.6 `recordCleared` ledger (campaign-only) + `openLore` stops re-locking. **Review fixes:** importSave now migrates; a new `_uiMode` decouples the menu/store mode from the live game's `S.mode` (opening a mode screen can't corrupt/mis-route a parked run); per-mode Merchant lock (`descentInProgress(mode)` + `quitToMenu` clears `_liveGame`+timer); setLevel-before-migrate on resume; campaign badge current-over-done.
...
1. `const LEVELS=[ {id:'warren', levelNo:1, name:'The Warren of Embers', subtitle, unlock:null, teaser, rooms:<the existing 3 DUNGEON rooms verbatim>, art:['grakk','murglax','vael']} ]` — room shape (name/villain/colors/hp/cmd/pool/lands/swaps/lore/intro/isVael/…) is UNTOUCHED so deck-building/AI/combat never notice. Future levels' enemies of OTHER colours (W/U/G) slot in as pure data — the room shape already carries `colors`, and the P44 mechanics library guarantees their slices (counters/draw/bounce/fight/…) are engine-ready; adding a level must never require engine work.
...
# PHASE 44 — Enemy realism & balance pass 2 ✅ DONE & merged (2026-07-05)

**BUILT & VERIFIED (each sub-task: jsdom driver + syntax gate + id-diff; phase gate: full regression + a 5-lens adversarial-review workflow, all 5 confirmed findings fixed).** P44.6 Gate-Meteor `mkPlay` fix (roll+rider) + removed the three dead knobs (`manaBonus`/`landStart`/`deckCopies`). P44.2 dual-mode `burn:N` (face OR lethal-to-a-creature by value; `resolveBurn`/`burnValue`/`burnKillables`; `_forceFace` for lethal, `_forceCreature` for a combat trick; 9 face-burn cards converted; `enemyLethalReach` counts burn) — the colour-agnostic creature-target route `fight` reuses. P44.4 the **colour-pie mechanics library** (built colour-agnostic so future W/U/G enemies are pure data): counter (hard + punisher `counterUnless:{life/dmg}` via `resolveCounter` + the P4.2 response flow, respects can't-counter) · `draw`/`rummage`/`discard` ops · `bounce`/`fight`/`naturalize` `applyTarget` routes (protection/ward-aware) · multi-kill SUM valuation. Level-I content colour-gated (Vael 1 punisher counter, easy-swap removes it; Murglax Sanguine-Study life-price draw + Mind Rot; Grakk one impulse draw; Vael Tormenting-Voice rummage); 4 `_`-prefixed library-only driver test cards in NO deck. P44.1 hold-up reserve (`enemyHoldUpReserve`, holds instants for the windows) + the mid-combat trick window (the deferred P4.4 — `enemyCombatTrick`/`approveCombatTrick`, one per combat). P44.3 walker AI (`pickWalkerAbility` ult-safety + defensive minus; `aiTargets` blocker-aware minimal-power split). P44.5 deck discipline (Murglax/Vael → 99, Grakk leaner 34-land aggro; `dealOpeningHand` mulligan v2 with a per-difficulty land window + curve/early-play tie-break; deeper swaps; `playEnemyLand` prefers flavored lands). **Review fixes:** punisher counter keeps its target on the stack (no autosave/reload data-loss) · resolvePlay resolves a pending responder first (non-LIFO counter no longer fizzles) · `aiTargets` prune (minimal-power divert) · a burn is held/tricked only for its reactive creature-kill (`reactiveBurnValue`), never a face-chip.



> **Goal.** Beyond the built P34/P35/P38/P17: the enemy should FEEL like a real opponent — holding up instants, tricking during your combat, using burn on creatures, defending its planeswalker, running interaction and card advantage the way its COLOURS really would — and the decks/difficulty should be disciplined, with the known bugs and dead knobs cleared. The phase doubles as the **colour-pie mechanics library** (user, 2026-07-05): interaction mechanics land NOW as colour-agnostic engine infrastructure so future levels' enemies of other colours are pure data; Level I decks take only the slice red/black really gets.
> **Colour-pie & card-provenance principles (bind every P44 sub-task and ALL future level content):**
> 1. **Mechanics ≠ content.** Every interaction mechanic (counter, draw, discard, bounce, burn-modes, fight, …) is built ONCE as a colour-agnostic op — FX shape + `applyRun` case + `castValue`/`bestTargetThreat` valuation + response-flow wiring where relevant — with its FULL option set, even when no Level I card uses it. Cards are the colour-gated content layer on top, so a future white/blue/green enemy is FX entries + pools + lore, ZERO engine work.
> 2. **Colour pie discipline** (extends P35.3's no-off-colour rule). Each deck takes only its colours' real slices. Level I is all-R/B: **R** = burn (face+creature) · haste · rituals · rummage/impulse draw · sac-for-value · at most a RARE punisher counter; **B** = removal · discard · drain · reanimation/recursion · life-price draw · at most a RARE punisher counter. Future slices the library must cover: **W** = lifegain/anthems/taxes/protection/wipes (mostly existing ops: `bossHeal`/`anthem`/`rule`/mass selectors) · **U** = hard counters/raw draw/bounce/clone/tap-down locks (counter+draw+bounce land in P44.4; clone/locks compose from `copyPermanent` + `rule`/markers) · **G** = ramp/big bodies/fight/artifact-enchant removal/trample (`ramp` exists; fight + art-removal are P44.4 library items).
> 3. **Card provenance.** Every new card is a direct adaptation of a real MTG card OR an original a real set could plausibly print in that colour — real cost/rate/type conventions, with the real-card model named in the FX entry's comment (P35.3's faithfulness rule, made binding from here on).

**Grounded findings (re-grep before building).** `vaelMain`'s cast loop (~2128) is **greedy — taps out every turn**; the instant windows (`enemyInstant` ~2189, `enemyRespondToCast` ~2429) can only spend leftovers, so interaction on your turn is accidental. The **mid-combat trick window is still the documented deferred P4.4 follow-up** (comment ~2188; `openCombat` ~1367 closes every proposal; `youCombat` ~2182 fires before attackers are even picked). `dealOpeningHand` (~895) guards screw ONLY and at the 10-attempt cap keeps the hand with the MOST lands (~901–902) — the inverse of real mulligan logic; `playEnemyLand` (~2007) plays the first land found. `pickWalkerAbility` (~2028) **ults unconditionally at threshold** and only minuses offensively vs a ≥5-power bomb (~2031) — no defensive line despite the P38.5 comment (~2024). `aiTargets` (~1694) is all-or-nothing on walkers (raw power ≥ loyalty, blocker-blind). **Zero counterspells** — `bestTargetThreat`'s `case 'counter'` (~2458) is dormant; the only draw effect is `cullharvest`; `destroyTop2`-style valuation scores only the best victim (`top()` ~2452). **All red burn is face-only `selfLoss`** (e.g. `FX.bolt` ~635) — Grakk has ONE removal card in 99 (`pyro` ~639) and no instant-speed removal; black removal is fixed heuristic selectors (power-based, not threat-based). **Gate-Meteor bug (live):** `FX.meteor` (~683) has `selfLoss:2` + `die:6/dieLoss:true` but `mkPlay` (~2239) overwrites `selfLoss` with the roll — the +2 never happens (documented at SPEC ~446 — the P6.2 "deferred to P5.2 balance" bullet; never fixed). **Dead knobs (confirmed):** `DIFF.manaBonus` (~615) and `room.landStart` (~779/785/791) are defined, read nowhere (retirement comment ~882); `deckCopies` (~622) only serves legacy array pools — all three rooms use maps. **Decks:** Grakk 99 (37 lands on a 2.35 curve — flood-heavy; ~33% is aggro-typical), Murglax **103** and Vael **100** (both over the 99 norm; avg cost 3.05 / 3.41); per-difficulty swaps are 1–3 cards. Special-run enchants like `bloodpact` still resolve into `S.rules`, not a destroyable permanent (~2307–2308). Phase 5's balance line still says "pending playtest". **Colour & op inventory (re-grepped 2026-07-05):** the wardens are Grakk `["R"]` (~779) · Murglax `["B"]` (~785) · Vael `["R","B"]` (~791) — Level I is an all-R/B level, so by the real colour pie counter-magic is at most a rare conditional splash. The enemy `applyRun` vocabulary is `spawn/bossHeal/cmdBuff/anthem/ramp/rule/bossrule/bloodpact/reanimate/recur/sac` (~2101–2111) — NO draw/counter/discard/bounce/fight op exists; enemy removal reaches ONLY creatures (the `destroyMaxPow`-family selectors; grep destroyArt|naturalize|shatter = 0 hits), so player artifacts/enchants are unreachable by enemy cards today.
...
## P44.4 — Interaction & card advantage: the colour-agnostic mechanics library + colour-pie'd Level-I content
1. **Counterspell mechanic (built full-strength NOW — blue's future default weapon):** a `counter` FX effect wired through the dormant `bestTargetThreat` case (~2458) + the P4.2 response flow (the enemy proposes countering your cast when its value clears the bar); respects "can't be countered" (P13.1). Two modes from day one: **hard** (counter outright — future blue's slice) and **punisher** (`counterUnless:{life:N}` / `{dmg:N}` — on approval YOU choose: accept the rider and your spell resolves, or let it be countered; real off-blue counters give the caster exactly that choice). **Hold-up interplay:** a counterspell has no stack target at the enemy's main time, so `castValue` would project ~0 — special-case its hold-up value to `enemyActThreshold()` (difficulty-scaled; 0 on easy) whenever the player holds ≥1 card, so P44.1's single reserve check sees it (the reserve logic stays defined ONCE, in P44.1).
2. **Counterspell content at Level I (colour pie: R/B counter-magic is RARE and always conditional):** default **Grakk 0 · Murglax 0 · Vael exactly ONE** punisher counter modeled on the real R/B family — Molten Influence (counter unless it deals 4 to the caster) / Dash Hopes (counter unless the caster pays 5 life) — e.g. "Tyrant's Refusal". NO hard counters in any Level I deck; easy's swap removes even the one. (Supersedes this task's earlier "1–2 'Dark Refusal' counters in Murglax + Vael" sketch — hard counter-magic is blue's slice, per the user 2026-07-05.) *Distribution flagged in Open questions.*
3. **Draw mechanic + colour-styled content:** a `run:['draw',N]` op; `castValue` values it by hand-size need. Content follows the pie — **Murglax** pays life (Sign in Blood model: "draw 2, the boss loses 2" via the existing `selfLoss` rider), **Vael** rummages (Tormenting Voice model: draw 2 + self-discard 1, reusing the P17.3 wise-discard picker — or an impulse-flavoured plain draw costed above rate), **Grakk at most one** cheap impulse (Light Up the Stage model) — red aggro barely draws. Raw unconditional draw (Divination-style) is blue's, library-reserved.
4. **Discard mechanic (black's other core slice — new op):** `run:['discard',N]` — resolves as a visible proposal; on approval YOU choose N cards from your hand to discard (the manual-hybrid convention; logged). `castValue` scales with your current hand size (worthless when you're empty-handed). Content: **Murglax 1–2** (Mind Rot / Hymn to Tourach family); no one else at Level I.
5. **Library-only ops (engine + driver coverage NOW, ZERO Level I cards — future colours' slices):** `bounce` (return a targeted player permanent to hand — blue's third core; rides the P9.1/`moveBoardCard` return machinery + P30 targeting protections). If the shared targeting keeps them cheap at build: `fight` (green — reuses P44.2's burn-at-creature route) and **player-artifact/enchant removal** (G/W Naturalize slice — unreachable today, see findings). Whatever isn't built stays specced here as one-`applyRun`-case-away; every BUILT library op ships with a driver-only test card, never a Level I pool card.
6. **Multi-kill valuation:** `destroyTop2`/board-sweep effects value as the SUM of victims (`top()` ~2452 → a sum), so 2-for-1s are preferred like a real player would.
**ACs:** the enemy counters a high-value player cast via the visible proposal (approvable/editable as ever) and the punisher choice is genuinely yours (both outcomes reachable); no Level I deck holds a hard counter and only Vael (default) holds the conditional one, gone on easy; draw and discard resolve with their colour styling; a sweep is preferred over a single kill when it nets more threat; hold-up (P44.1) reserves for counters too; every BUILT library op resolves cleanly from a driver test card despite no Level I deck carrying it.
**Verify:** jsdom — hard counter (driver card) + punisher both paths (pay-rider-resolve / decline-countered) + fizzle on can't-be-countered; pool audit per difficulty (0 counters Grakk/Murglax · 1 Vael · easy-swap removes it); life-price draw + rummage + discard resolve; bounce (+fight/art-removal if built) driver cards resolve; sum-valuation ordering; syntax + id-diff.
...
3. **Deeper difficulty swaps:** beyond the current 1–3 cards — easy cuts the top-end finishers; brutal swaps clunkers for gas (± the interaction package, always colour-legal per the phase principles) so difficulties play differently, not just tankier (room.swaps ~781/787/797).
...
### ✅ VALIDATION RECORD — 2026-07-06 (Phase 46 Step 3 / final gate)

1. **Full regression — GREEN.** `npm test` → **24/24** (syntax gate · id-set guard 258 · SW-drift guard · full-descent · both-modes/all-difficulties · 15 system + 4 P46 Step-2 + cleanup drivers). Offline, deterministic (multiple re-runs). Both game modes and all three difficulties drive a full run to a natural end with **zero console errors** (`modes-difficulties.test.js`: campaign easy+brutal + a sandbox battle; `full-descent.test.js`: campaign standard → 3 rooms → Vael reborn → win).
2. **Syntax + id-diff — CLEAN.** `vm.Script` compile OK. Cumulative id-set diff vs the pre-rework base (`0af2bb0`, Phase 40) shows **only additions** except one intentional removal — `menuGold` (0 references; retired in P43's per-mode-gold rework). No dangling ids.
3. **Live-browser smoke — PASS.** Headless MS-Edge renders the current build at **1360px** (3-rail Moxfield grid, warden portrait, ⚙ Settings in the controls bar) and **380px** (unchanged accordion phone stack); the ⚙ Settings panel renders (volume/text-size/animation/data). **Live Scryfall online import verified** against the real API — search → `buildImportedCard` maps Lightning Bolt (instant · cost 1 · dmgBoss 3) and Goblin Guide (2/2 haste), and the P45.2 `image`/`art` URLs are captured from live JSON. **Offline typography guaranteed** — the 4 typefaces are inline data-URIs in the SW-cached shell (zero external font requests); art degrades to text tiles offline. **Legacy-save migrate verified** — a pre-41 save (flat gold 250, `S.pw`, no camp/sand) loads with gold→`camp.gold` intact, `S.pw`→a real `enemyWalker`, `seenTutorial` backfilled, library preserved, zero errors.
4. **Final multi-agent adversarial review — DONE.** A 3-lens review (cross-phase integration · save/migrate · completeness) of the whole shipped state found **2 confirmed integration bugs** (the kind a per-phase review can't see), **both fixed** (3 more refuted): (MED) **Esc at the title menu** fell through the a11y handler's `_A11Y_CLOSE.menu=null` to a generic `remove('show')`, exposing the phantom non-live boot-time board drawn under the menu — fixed to treat an explicit-null as focus-trapped-but-not-Esc-closable; (MED) the **Merchant affordability** check (`renderStore`) used `getGold()` (live `S.mode`) instead of the browsed `getGold(_uiMode)`, locking the Sandbox store out of Sandbox gold after a reload — fixed to the browsed purse. Covered by `tests/p46-8-review-fixes.test.js` (9/9).
5. **Balance playtest — HANDED TO USER (below).** The engine is validated; per-difficulty *balance* (clear-rate, gold pressure, item usage) is data only real play can produce.
6. **Deploy / PWA — files DONE, connection BLOCKED-ON-USER.** `netlify.toml` (publish root, no build, no-cache SW/shell headers) + `sw-version.test.js` drift guard are in-repo. The one-time **Netlify Import-from-Git** connection is the user's action (checklist below); until then drag-drop stays the fallback. Installed-PWA refresh model verified in-design (network-first nav + bumped `gg-cache-v49`).

### 📋 BALANCE PLAYTEST CHECKLIST (for the user — fill in and paste back)

Play each difficulty a few runs and note:
- **Clear rate** — of N Level-I attempts, how many reached/beat Vael? (target rough feel: Easy ~forgiving, Standard ~50/50, Brutal ~punishing.)
- **Where do runs die** — which warden (Grakk / Murglax / Vael), and to what (aggro rush · attrition · Vael's Avatar ult · deck-out)?
- **Gold pressure** — could you afford the items you wanted between descents, or always broke? Any item never worth buying?
- **Item usage** — which loot/store items did you actually use vs. ignore? Any passive that felt useless or mandatory?
- **Enemy feel** — did the AI ever do something obviously dumb (bad block, wasted removal, no-op turn) or unfair (impossible curve)?
- **Anything that felt broken, confusing, or un-fun.**

### 🚀 DEPLOY CHECKLIST (for the user — one-time)

1. Netlify dashboard → **Add new site → Import an existing project** → pick the `Guffs-Gauntlet` GitHub repo.
2. Build command: **(empty)** · Publish directory: **`/`** → Deploy. (`netlify.toml` already encodes this.)
3. After that, **every `git push` to `main` = an automatic deploy** to the same URL (installed players auto-update on next online launch).
4. On each future build: bump `gg-cache-v*` in `sw.js` **and** the matching line in `README.md` (the `sw-version` test enforces this) — then push.
...
> - **Three bullets are already satisfied — treat as verify-only, don't rebuild:** #2 (instant/sorcery→graveyard both sides — P24.1/P7.2 already do it), #10 (resurgent already fires at end of main-1 and skips when creatures are on board), #13 (unused *consumables* already persist across losses **and** levels via the P15.3 stash — user-confirmed 2026-07-07 this is the whole ask; passives correctly expire on descend, no change).
> - **Three bullets reverse an earlier, deliberate decision — do NOT silently overwrite; each is called out in-line and echoed in `## Open questions`:** #16 enemy **coloured mana costs** (P44 open-Q default was "NO, costs stay generic numbers"), #20/#21 **automate enemy instants** (reverses P4.2/P4.4 "the player approves/edits the enemy's play"), #22 **PW abilities off the stack** (reverses P38 "abilities resolve as stack items so the player can respond").
> - Line numbers are **as of the P48.3 build (`6e438f1`) — re-grep before trusting them**; the `ART` blob sits at ~683 so JS line numbers are ~+70 vs the old `index.html`.

## Build record (2026-07-07 — what actually landed)

**🟢 10 of 12 sub-tasks BUILT, merged & pushed to `origin/main` (@ `2714681`).** User directive: "implement the easy tasks, leave out the mega tasks, commit-merge after each, continue until end." Built in the budget-first order below (small → medium → large), branch-per-task (`build/p49-N`) → merge `--no-ff` → push, each with a committed `tests/p49-*.test.js` driver + syntax gate; the risky ones (P49.2 / .3 / .10) each got a single-agent adversarial review. **`npm test` = 35/35 green.**

- ✅ **P49.1** — collapsible enemy mana-override sub-box (`S.ui.manaBox`/`applyManaBox`) + 44px warden icons + Library sections default-collapsed.
- ✅ **P49.2** — `renderPlays` newest-on-top (display-only, true index preserved) · countered player spell → `S.myGy` · **commander tax = real MTG** (`cost=base+CMD_TAX_BASE*czCasts`, bumps only on a command-zone cast; hand casts untaxed; death/bounce/counter don't bump — removed the 3 per-death increments, deleted `CMD_TAX_DOUBLE`, relabelled "deaths"→"CZ casts").
- ✅ **P49.3** — enemy instants **auto-cast** via `_castEnemyInstant` (**reverses P4.2/P4.4**); still counterable, combat-proposal path intact, instant-proposal UI dormant.
- ✅ **P49.4** — wired the callerless scope-aware `setCtr` (counters on artifacts/enchantments both sides + walker drawer; new `ctrCustom`/`permCtrBtns`/`permCtrChips`).
- ✅ **P49.5** — WHEEL slices strictly-decreasing (35/25/20/12/6/2) · `goldReward` 12+r·10 → 8+r·6, brutal mult 1.3→1.15 · Mana Surge +2→+1/turn, price 22→30.
- ✅ **P49.7** — final-level "To be continued…" ending gated on `S.levelIdx>=LEVELS.length-1` + `abandonCampaign()`.
- ✅ **P49.8** — colour-loyal naturalize per warden (`grakksmash`/`murgrot`/`vaeldisjunct`) + Grakk ramp artifact `emberidol` + deeper difficulty variation (easy 0 / std 1 / brutal 2 relic-removal).
- ✅ **P49.10** — combat popup minimize/restore (`S.combat._min` + `#combatFab`, turn stays blocked) · **enemy walker abilities resolve INLINE off-stack** (`_resolveAbilityInline` + toast, **reverses P38**, enemy-only) · side-selectable battles (`addBattle('you'/'boss')` + row toggle) · resurgent skips when a creature is pending.
- ✅ **P49.11** — Vael reborn 1/1/1 HP · Ash +1 → 1/1 haste Ash Soldier · Ash −3 → `ashReanimate` graveyard reanimator + AI fizzle guard `_ashReanimBest`.
- ✅ **P49.12** — deck-tools reveal rows use expandable `fxItem` · homebrew card Art URL input → `cfg.art`, edit round-trips art.

**⬜ NOT built — the 2 mega-tasks the user excluded (each wants its own weekly budget window; narrow via the recommended defaults in `### Phase 49 decisions`):** P49.6 (Italian i18n layer, net-new) · P49.9 (enemy coloured/colourless mana + London-mulligan penalty).

## Execution plan (set 2026-07-07 — budget-aware)

**Weekly-budget reality.** The user was at **84% of the weekly usage limit on 2026-07-07** (~16% left). All 12 sub-tasks with the full multi-agent adversarial-review workflow on each do **not** fit that remaining budget — the two mega-tasks (P49.6 i18n, P49.9 coloured mana) alone would blow it. So Phase 49 is built **budget-first, by cost triage**, and may span multiple weekly windows. Do NOT try to land everything in one push.

**Build order (cheapest, highest value-per-token first; stop-and-ask "continue?" after each sub-task, branch-per-phase as always):**
1. **Small/contained:** P49.1 · P49.7 · P49.11 · P49.12 (+ the free verify-only asserts: bullet #2, #10; #13 dropped).
2. **Medium:** P49.2 · P49.4 · P49.5 · P49.10.
3. **Large:** P49.3 · P49.8 — only if budget remains.
4. **Mega (defer to their own weekly window each, narrow via the recommended defaults):** P49.9 (**heuristic pips, Vael-only**) · P49.6 (consider **excluding the Dungeon Log**, which ~halves it).

**Questions are asked INLINE, not upfront.** The user chose (2026-07-07) to answer the `## Open questions` / `### Phase 49 decisions` items **during implementation** — when a sub-task with an open decision is reached, pause and ask that sub-task's question(s) right before building it, then proceed. Do NOT batch all decisions into one pre-flight questionnaire; do NOT block the cheap tasks waiting on the mega-task decisions.

**Budget hygiene per task.** Keep the per-task workflow (synt.js gate → id-diff → jsdom driver → commit → push via `npm test`), but **scale review effort to risk to conserve budget**: reserve the full multi-agent adversarial-review *workflow* for the risky sub-tasks (state/combat/copy/core-stat: P49.2, P49.3, P49.9, P49.10) and use a single-agent or self-review on the contained data/UI tasks (P49.1, P49.5, P49.7, P49.11, P49.12). Commit each sub-task the moment it's green so progress survives a mid-batch budget stop.

## P49.1 — Enemy-box UI: collapsible mana-modifier sub-box + ⓘ · bigger warden icons · Library sections default-collapsed  *(bullets 1 · 5 · 4)*

**Bullet 1 — group the enemy mana modifiers into a collapsible, info'd sub-box.** *Grounded:* the enemy box is `.panel.boss` (~437–449). Its **read-only** readout is `#manaPips`+`#manaLine` (~441, rebuilt by `render()` at ~1703–1709, the P37 lands-only readout). The **manual override** controls are three static `.btnrow`s beneath: pool (`bossManaAdj`/`#bossPoolVal`/`bossManaSet`/`bossManaEmpty`, ~444), per-turn modifier (`bossManaModAdj` ~2108 / `#bossManaModVal` / `bossManaModSet` ~2109, ~445), and freeze (`bossFreezeAdj` ~2114 / `#bossFrozenVal` / `bossFreezeClear` — already carries its own `showInfo('freeze')` ⓘ, ~446). *Change:* wrap the three override rows in a new collapsible sub-box (header row + a `.pchev` chevron + `infoBtn('enemymana')`), keeping the read-only `#manaLine`/`#manaPips` **always visible**. **Do NOT reuse the P12.1 `data-panel` idiom** — `applyPanels`/`defaultPanelStates` (~3579–3581) only collapse whole `.panel[data-panel]` boxes, and the boss panel is deliberately left non-collapsible (Info you/foe duo, comment ~3566). Instead mirror the lighter Dungeon-Log idiom: a `S.ui.manaBox` flag (**default collapsed** — "to save space"), `applyManaBox()`/`toggleManaBox()` (toggle a class + `scheduleAutosave()`), called beside `applyLog()`/`applyPanels()` in `render()` (~1725); CSS to hide the body when collapsed (analogous to `.panel.collapsed>:not(h2)` ~368); a new `INFO_TEXT.enemymana` entry. **The ids `#bossManaModVal`/`#bossPoolVal`/`#bossFrozenVal`/`#manaLine`/`#manaPips` must remain in the DOM** — `render()`'s null-guarded updates (~1703–1710) silently skip if any is missing, so the wrapper must *contain* (not replace) them.

**Bullet 5 — bigger warden icons.** *Grounded:* `ENEMY_ICON` (~905, the P47 per-warden `Pictures/*.jpg`) feeds the one `#bossPortrait` in the boss `<h2>` (~438); size is the single CSS rule `.wardenpt` at ~86 (`34px`). *Change:* bump to ~42–44px (one rule, all three wardens share it; nudge `border-radius` ~7→8px). Watch the `<h2>` line-box / narrow-width wrap (it also holds the name, colour dots, `⚜ Abilities`, frozen badge). Primarily a visual pass.

**Bullet 4 — Library "Cards" & "Tokens" start collapsed.** *Grounded:* P27.1 `applyLibSections(searching)` (~3395) currently opens on undefined (`lib.cards!==false`); `toggleLibSection` (~3398) lazy-inits `{cards:true,tokens:true}`. *Change:* flip the unset default to **closed** — `applyLibSections`: `cardsOpen=searching||lib.cards===true` (etc., **keep the `searching||` live-search auto-reveal**); `toggleLibSection`: init `{cards:false,tokens:false}`; update the P27.1 comment (~3394). Players who explicitly opened a section keep it (`===true`).

**Verify:** jsdom — new mana sub-box exists + collapsed by default + `S.ui.manaBox` reflects it; `toggleManaBox()` flips class+flag+schedules save; `bossManaModAdj(1)` still updates `#bossManaModVal` (proves ids survived); `showInfo('enemymana')` opens the info modal. Library: fresh `openLibrary()` → `#libCards`/`#libTokens` `display:none`, chevrons `▸`; a `#libSearch` query reveals both; a toggle opens + persists. `.wardenpt` size is a visual/rule check.

## P49.2 — Spell/stack routing: newest-on-top · countered spell → graveyard · commander tax flat +2 per **cast**  *(bullets 2 · 12 · 15)*

**Bullet 2 (instant/sorcery → graveyard, both sides) — ALREADY DONE, verify only.** Player: `resolvePlayerItem` (~2531) pushes a `{name,color,ctype,_spell:true}` record to `S.myGy` (P24.1). Enemy: `resolvePlay` (~2522) `S.gy.push(p._card)` for any non-`enchant` resolved card (P7.2); counterspells route at ~2520. No change beyond a regression assert. The *only* adjacent gap is the **countered** player spell (bullet 15A).

**Bullet 12 (new spells on TOP) — display-only.** Mechanically the stack is already correct LIFO: new plays `S.plays.push(...)` (newest = highest index) and `resolveStack` (~1494) iterates `length-1→0` (newest resolves first); the `topIdx` "TOP ▸" badge (~2460/2471) marks it. But `renderPlays` (~2459) renders `S.plays.forEach` (oldest-first), so the newest card visually sits at the **bottom**. *Change:* in `renderPlays` **only**, iterate `for(i=len-1;i>=0)` so visual top = stack top — **keep `i` = the true array index** passed to every handler (`resolvePlay(i)`, `resolvePlayOn(i,…)`, `counterPlay(i)`, `editStackItem(i)`, `copyStackItem(i)`, `removeStackItem(i)`, `saveStackToLibrary(i)` + the single-target `<select>`/`stackEditor`). Do NOT touch `push`/`resolveStack`.

**Bullet 15A (countered spell → graveyard / command zone).** Player-counters-enemy `counterPlay` (~2699) already routes `_enemyCmd`→enemy command zone else `S.gy.push(p._card)`. Enemy-counters-player `counterPlayerSpell` (~2702) routes `_pcmd`→`S.pcmd` (command zone) but a **countered non-commander player spell vanishes** — no `S.myGy` push. *Change:* in `counterPlayerSpell` (and its `counterChoiceDecline` path ~2719), for a non-commander player spell push a `{name,color,ctype,_spell:true}` record to `S.myGy` (mirror ~2531). **Bullet 15B (commander tax = real MTG — DECIDED by the user 2026-07-07).** *Grounded:* constants `CMD_TAX_BASE=2, CMD_TAX_DOUBLE=true` (~690) → **doubling** increments (2,6,14); tax accrues on **death only** (`enemyWalkerDies` ~2246, `removeRef` ~1311, `sendCmdToZone` ~1836); casting doesn't bump — so a countered/bounced commander is never taxed (contradicts real MTG). *Change — model it exactly like real Commander:* the tax is **+2 for each time this commander has PREVIOUSLY been cast FROM THE COMMAND ZONE this game** (flat, cumulative: base+0, base+2, base+4, …). Concretely:
> - Set `CMD_TAX_DOUBLE=false` (flat +2 steps; `CMD_TAX_BASE` already 2). Track a per-commander *command-zone cast count* (reuse/repurpose the existing `cmdTax`/`tax` field, or a `czCasts` counter × `CMD_TAX_BASE`); cost = base + `CMD_TAX_BASE`×(previous CZ casts).
> - **Increment ONLY on a cast from the command zone** — bump at cast time in player `castCmd` (~1841) and enemy `vaelMain`'s cmd-cast branch (~2324, after `S.bossMana-=cc`), **after** computing this cast's cost.
> - **A cast from HAND is untaxed and does NOT increment the counter** (real MTG: the tax only counts command-zone casts). So a commander that was bounced to hand (P29's `cmdInHand`) casts at base and leaves the counter untouched.
> - **Death / bounce / counter do NOT raise the tax by themselves** — they just return the commander to the command zone at the already-accrued tax → the next command-zone cast is +2 higher than the last one. **Remove** the three per-death increments (~2246/~1311/~1836).
> - **"Deploy direct"** (`deployCmd` ~1840, the stack-bypassing shortcut) is a UX alias for casting from the command zone → treat it as a CZ cast (taxed + increments) so the shortcut can't dodge tax.
> - Same model for the enemy commander (Ash the Guardian is a *walker* commander — its cast tax follows the same CZ-cast rule).

**Reconciliation:** 15B reworks the **P29.1** per-death tax model + its "deaths N" UI reminders (`renderPlayerCmd` ~1843–1845, `cmdFieldCard` tax chip ~2427/2433, enemy cmdbox ~1655) — relabel those "deaths" chips to **"CZ casts"** (they no longer drive the tax; a death only matters because it returns the commander to the command zone). `freshGameForDungeon` (~2442) already resets `cmdTax`/`cmdDeaths` per room — keep (a new dungeon = a new game = tax resets). **P29.1 command-zone-vs-hand distinction is preserved and is exactly what enforces "from hand = no tax".**

**Verify:** jsdom — `castCmd()` → a `_pcmd` play on `S.plays` and `S.pcmd.cmdTax===2`; then `counterPlayerSpell(thatPlay)` → `S.pcmd` set, `cmdTax` still 2 (not 4); `counterPlayerSpell` on a `{ctype:'instant'}` → `S.myGy` grew; enemy `vaelMain` cmd-cast → `S.cmd.tax` +2. Stack: two casts → the first-rendered `.playcard` is `S.plays[last]` and carries `.topbadge`.

## P49.3 — Enemy instants fully automated (spend mana + card→graveyard) + expandable enemy stack spells  *(bullets 20 · 21)* — **reverses P4.2/P4.4**

*Grounded:* enemy instants are **proposed, not automated** — `enemyRespondToCast` (~2639) / `enemyInstant` (~2394) build a `S.stackProposal`; the human clicks `approveStackProposal` (~2694, "You approve the enemy's play" ~2691) to actually cast. That approve path **already** enforces both invariants the user asks for: `S.bossMana-=fx.cost` **and** `S.hand.splice(idx,1)` (removes the card) → `p._card=card` → on resolve `resolvePlay` routes it to `S.gy` (~2522/2520). *Change (bullet 20/21):* have `enemyRespondToCast`/`enemyInstant` **directly cast** the best candidate when its `castValue` (from `buildEnemyCandidates` ~2638) clears `enemyActThreshold`, replicating `approveStackProposal`'s body (deduct mana, splice hand, `mkPlay`, set `p._card/_instant/_respondingTo`, `S.plays.push`, `sfx('enemycast')`) instead of setting `S.stackProposal` — **preserve the mana-spend + card→graveyard invariants**. Keep `showPlays()` so the player can still counter it. **Expand:** the effect text is **already shown** in `renderPlays` (~2471, `p.text||p.note||playerItemDesc`), and enemy plays carry `p.text=fx.text` — so ensure every enemy-instant FX has a `text`; if a dedicated read-only expander is wanted, reuse the ✎ `stackEditor` drawer or add a details toggle.

**Reconciliation / cautions:** reverses **P4.2/P4.4** (player-approved enemy responses). `S.stackProposal` is **also** used for combat proposals (context `'combat'`) and instant-speed windows (`'window'`) — `proposalLive()` (~2681) / `renderStackProposal` gate on context; **do not break the combat path**. Decide whether to fully remove the instant-proposal UI (`renderStackProposal`/`approveStackProposal`/`passStackProposal`/`setProposalChoice`) for the cast/window contexts or leave it dormant. **Open Qs:** should the player keep a counter step (assumed yes — auto-cast still counterable)? should **proactive** windows (`enemyInstant`) also auto-fire or only responses to a player cast?

**Verify:** jsdom — seed `S.hand` with an instant FX + enough `S.bossMana`, set a legal window, `enemyRespondToCast(playerItem)` → assert (a) `S.stackProposal===null`, (b) `S.plays` gained an enemy instant with `_respondingTo===playerItem.id`, (c) `S.bossMana` fell by `fx.cost`, (d) `S.hand` lost the card; then `resolvePlay` that item → `S.gy.length` +1. Combat-proposal path still works.

## P49.4 — Counters on artifacts/enchantments (both sides) + battlefield UI for non-creature permanents  *(bullet 3)* — completes P30

*Grounded:* creatures get a full inline `.ctr` counter row on every board (player `renderMy` ~1740; enemy tokens `enemyCard` ~1649; enemy commander `cmdFieldCard` ~2436) via `myctr`/`cctr`/`cmdCtr` + `*Custom` + `remCtr(scope,id)` + `remCtrOther`. **Artifacts/enchantments expose NONE** — player `fillPerm` (~1745) and enemy `renderEnemyEmblems` (~2050, scopes `eart`/`eench`) render a terse `.brow` with no counter controls and never show `plus`/`minus`/`other`. Crucially the plumbing is already built and scope-aware: **`setCtr(scope,id,kind,n)` (~1780, P30.6) exists but has ZERO callers**; `remCtr`/`remCtrOther` are scope-generic; `getObj` (~1753) already resolves `artifacts`/`enchants`/`eart`/`eench`/`cmd`/`token`. *Change (UI-only):* (1) player — in `fillPerm` add a counter group next to the mana/dmg inputs calling `setCtr('${cat}',id,'plus'/'minus',1)` + a new **scope-aware** `ctrCustom(scope,id)` prompt + `remCtr('${cat}',id)`, and render `p.plus`/`p.minus` + `(p.other||[]).map(...remCtrOther)` chips (copy the ~1739 pattern); (2) enemy — same in `renderEnemyEmblems`'s perm groups using scope `eart`/`eench`; (3) add the one `ctrCustom(scope,id)` helper (the existing `myctrCustom`/`cctrCustom` hard-code creatures/tokens); (4) optional parity: `setCtr` should `sfx('counter')` like `myctr` does.

**Reconciliation:** completes **P30** parity (it added protection/hexproof/shroud/indestructible/markers via `commonPermRow` but left tracked **counters** out; `setCtr` was written scope-aware for exactly this and never wired). Consistent with **P14.4** `bulkCtr` (already writes `plus`/`minus` to these) and distinct from P14.2 status markers (booleans in `other`). Enemy artifacts/enchants are real permanents (P13.3/P14.3) so `getObj` round-trips them — no migration. **Open Qs:** placement — inline on both boards, or into the ✎ `enemyPermDrawer` for the terse enemy row (which has no mana input)? include +1/+1 −1/−1 on P/T-less permanents (lean yes, for `bulkCtr` parity) or only the custom named counter? include **walkers** (their drawer ~1822 also lacks a named-counter control) in the same pass?

**Verify:** jsdom — push a player artifact + an enemy enchant; `setCtr('artifacts',id,'plus',2)` → `plus===2`; `ctrCustom`→`other` gains the label; `remCtr('eench',id)` pops; `remCtrOther` removes a chip; diff `#myArt`/`#myEnch`/`#enemyEmbList` innerHTML to confirm the controls now render; regress P30 parity + `bulkCtr('all-permanents')` + `resetCard`.

## P49.5 — Economy retune: Fortune Wheel · post-battle gold · Mana Surge  *(bullets 6 · 18 · 30)*

**Bullet 6 — Fortune Wheel rarer-but-weaker inversion.** *Grounded:* `WHEEL` (~961) d100 slices = g50 25% / g100 30% / g150 23% / g2x 12% / rare 7% / legendary 3%; `spinWheel` (~966) pays g50 =½base, g100 =base, g150 =1½, g2x =2×, rare/legendary =`pickByRarity`. The low end is **non-monotonic**: the *weakest* prize (g50, 16g at base 32) is drawn **less** often than the better g100 (25% vs 30%). *Change:* re-widen the slice bounds in the `WHEEL` array (**line 961 only** — `spinWheel`/`wheelSliceFor`/`wheelHTML` read it generically) so width **strictly decreases** as reward rises (e.g. g50 [1,35] · g100 [36,60] · g150 [61,80] · g2x [81,92] · rare [93,98] · legendary [99,100]); everything g100-and-up is already monotonic, so only g50/g100 is broken. Note the finale always spins at `goldReward(2)` (base 32, single-level), so g50=16g can undercut clearing mid-boss Murglax (22g) — consider a floor. **Co-tune with bullet 18** (`wheelBase()=goldReward(S.roomIndex)`).

**Bullet 18 — post-battle gold too steep.** *Grounded:* `goldReward(roomIdx)=12+roomIdx*10` × `DIFF.goldMult` (~957; easy .8 / std 1 / brutal 1.3) → std 12/22/32 per room. Gold **banks** on `p.camp.gold` across runs (never reset; `restart` doesn't clear it) and the store tops at 42g, so one ~66g std clear funds 2–3 items and a few runs buy out the shop → the plausible "too steep". *Change:* lower the base `12` and/or the `*10` per-room slope and/or compress the brutal `1.3` (e.g. `8+roomIdx*6`). `goldReward` is the **single choke** feeding mid-boss gold (`bossDown` ~2163), the wheel, and sandbox gold — one edit rescales all; decide whether the final room grants a flat base at all (today `win()` pays zero flat — all finale gold is the wheel).

**Bullet 30 — Mana Surge overpowered.** *Grounded:* `BOONS.surge` (~929) = rare, 22g, `kind:"reminder"` → **no engine automation** (the player has no mana pool; it's honor-system). Effect: **+2 mana every turn** for a whole descent, always-on, no card/land cost — strictly larger than the sibling Scholar's Token (+1 card/turn @18g). *Change (data-only, `BOONS.surge.t`/`.r` ~929 + `STORE` cost ~948):* weaken/bound — +1/turn, or cap to the first 2–3 turns (like Arcane Tome), or once-per-descent burst, or bump rarity→legendary / price ≥36g. Note `surge` is in `pickByRarity('rare')` so nerfing it also softens the wheel-rare pull.

**Reconciliation:** wheel/`spinWheel` are P15.1; store **prices** were tuned in P42.4 (income was not) so retuning income is complementary — keep the price/income ratio in band. **Open Qs:** target gold-per-clear; income (`goldReward`) vs price lever; exact wheel spread + g50 floor; Mana Surge nerf shape (and: should player mana become an engine-enforced resource, or stays honor-system?).

**Verify:** jsdom — loop `wheelSliceFor(r)` r=1..100, assert per-tier counts **non-increasing** across [g50…legendary]; `spinWheel(roll,32)` at boundaries returns the right tier/gold; `goldReward(0/1/2)` per difficulty matches the new table; a full-descent driver's `getGold()` delta lands in the target band; `BOONS.surge`/`STORE` reflect the nerf and `grantBoon('surge')` still works.

## P49.6 — Italian localization + main-menu flag/language selector (default EN)  *(bullet 7)* — net-new i18n layer

*Grounded:* **there is zero existing localization layer** (`grep i18n|locale|lang|translate` finds nothing) — all copy is inline English literals. Global toggles already live on `DB` (`DB.vol`/`DB.pad`/`DB.settings`), applied at boot via `loadSettings()` (~3588, boot ~3669); the menu button row (~623) is the natural home for a control; `setTextSize` (~3585) is the exact precedent (mutate DB → `saveDB` → apply → re-render). Broken into five deliverables:

- **7a — infra.** `DB.lang` default `'en'`; a helper (`t(key)` reading a locale-keyed map, or the lighter `L(en,it)=>DB.lang==='it'?it:en` per literal); `applyLang`/`loadLang` wired into `loadSettings`; a **flag control** in the menu row (~623) modeled on `setTextSize`. **Read-time selection** for mid-game copy (functions that rebuild innerHTML per call swap live); `location.reload()` after `saveDB` is the safe option for **static-at-load** chrome (autosave + Continue make it lossless).
- **7b — narrative.** Italian parallel copy for `DUNGEON` lore/intro/quote/deathQuote + Vael's `vaelEscape`/`guffFreed`/`epilogue` (~857–895), `ARRIVAL_LINES` (~918), `LEVELS` name/subtitle/teaser (~898), and `openLore` inline prose (~2905–2915). **Keep every proper noun** (Grakk/Murglax/Vael/**Ash the Guardian**/Guff; Ashveil/Warren of Embers/Ember Gate/Drowned Conclave…).
- **7c — help systems.** `TUTORIAL_HTML` (~2956, the largest single block), all 12 `INFO_TEXT` `{t,h}` (~2829), `ENEMY_ABIL` + the inline literals in `enemyAbilitiesHTML` (~2849 / ~2879–2900). `openTutorial`/`showInfo`/`enemyAbilitiesHTML` rebuild innerHTML per open, so read-time selection needs no reload.
- **7d — static chrome.** Header/help panel (~393–416), the **4 tab labels+notes** (~419–422; `buildTabs`' `dataset.built` guard ~3554 blocks a re-run, so a live switch needs an explicit DOM rewrite or a reload), the full menu DOM (~602–628, **keep the "Guff's Gauntlet" title**), campaign/sandbox/library/store overlays, `settingsHTML` (~3589), `PHASE_LABEL` (~689 via `flowLabel`).
- **7e — shop + inline UI.** `BOONS.t` (**keep `BOONS.n` names**), `STORE` category labels, rarity words, satchel labels; the inline literals in `renderStore`/`renderMenu`/`renderCampaign`/`renderSandbox`/`flowLabel`/cutscene buttons/alerts. **Decide the ~305 `log()` narration** (the biggest bucket, heavily name/`${}`-interpolated) — in scope or out (this roughly halves the total effort).

**Reconciliation:** reuse the P46.3 Settings `DB.settings`/`setTextSize`/`loadSettings` pattern; intersects P48 tab labels and P43 mode screens. **Rules to honor (user's own):** translate cutscene/lore but NOT names; UI/tabs/boxes/tutorial/info/abilities/buttons but NOT the optional per-card controls; shop/menus but NOT names or the main title; Library texts but NOT the cards (real Scryfall + saved card names stay original); Settings. **Open Qs (surface before building):** language **global vs per-profile**; **reload vs fully-live** switch for static chrome; **is the Dungeon Log in scope**; do MTG rules keywords (commander/planeswalker/deathtouch/trample) stay English.

**Verify:** jsdom — `DB.lang` defaults `'en'`; `setLang('it')` persists + swaps anchor strings (menu subtitle, a `PHASE_LABEL`, a tutorial heading, an `INFO_TEXT`, a store item's `t` while its `n` stays English) with proper nouns preserved; the `<h1>` title untouched; toggling back restores EN; no console errors; an autosaved run survives the switch.

## P49.7 — Campaign flow: Level-1 "To be continued…" + replay-lock + cancel campaign  *(bullets 8 · 29)*

**Bullet 8 — "To be continued…" after the final level; replay only via a NEW campaign.** *Grounded:* `LEVELS` (~898) has **exactly one** entry — Level I is first *and* final, so the between-level super-advance (`levelClearTransition`/`levelClearCutscene`/`descendToNextLevel` ~2763–2776) is **dormant** (P43.4 comment ~2759). Live path: clearing Vael → `bossDown` (~2160) → `win()` (~2724) which `saveUnusedItems`+`clearSave` (nulls `p.camp.run`) + `recordCleared` + `bumpLevelUnlock`, then `openWheel`→`winCutscene` (3-beat vaelEscape/guffFreed/epilogue). BEAT 3 buttons (~2752) = `[↻ Descend again → restart, Quit to Menu → quitToMenu]`, and `restart()` (~2816) **immediately replays Level I**. *Change:* replace BEAT 3's two buttons with a single **"To be continued… ▸"** wired to `quitToMenu` (or `closeCutscene`+`showMenu`). **Do NOT delete `restart()`** — still used by `lose()` (~2793 "↻ Try again") and `#resetBtn` (~2818). Level 2 is **already inherently blocked** (no `LEVELS[1]`; `win()` clears the run so the menu Continue drops the campaign and `renderCampaign` shows "✦ New run" → a fresh Level I). **Recommend** gating the swap on `S.levelIdx>=LEVELS.length-1` so it auto-reverts to the normal `levelClearCutscene` ("Descend to Level 2 ▸") once a real Level 2 ships.

**Bullet 29 — cancel/abandon the current campaign.** *Grounded:* `renderCampaign` (~3198–3214) current-level row (~3208) offers only Continue (`continueCampaign`) / New run (`campaignStart`→`confirmNewDescent`). The run lives only at `p.camp.run`; the rest of `p.camp` is persistent progression (`gold` purse, `stash`, `pending` Merchant purchases, `cleared` unlock ledger, `level`). *Change:* add a **"✕ Abandon campaign"** button → new `abandonCampaign()` that resets **run position ONLY** (`p.camp.run=null; saveDB(); renderCampaign()`), preserving gold/stash/pending/cleared/level. Gate behind a confirm cutscene (model `confirmNewDescent` ~3178). **Pass the mode explicitly** — the bare `clearSave()`/`setSave(null)` default to the live `S.mode`, which from the menu can be a stale `'sandbox'`.

**Reconciliation:** P43.4 separates the campaign-end path (`win`→`winCutscene`, edited here) from the dormant between-level path (untouched); P43.1/43.3 per-mode buckets (abandon uses the same `p.camp.run` slot). Fix 8 (win nulls the run) and fix 29 (abandon nulls the run) reach the same end state — aligned. **Open Qs:** exact button copy; keep a 2nd button (lore re-read)?; update the epilogue **text** to signal a pause?; abandon scope (run-only vs also refund `pending`/wipe purse); confirm dialog vs one-click.

**Verify:** jsdom — drive `win()`→claim wheel→`winCutscene` BEATs, assert BEAT 3's `#cutBtns` has exactly one "To be continued" action and no "Descend again", and its click leaves `#menu` shown with `getSave('campaign')`/`prof().camp.run===null`; `renderCampaign` then shows "New run" not "Continue run". Seed `p.camp` (run+gold+cleared+stash), `abandonCampaign()` → `run===null` but gold/cleared/level/stash unchanged, tile re-renders beginnable, `hasSave('campaign')===false`.

## P49.8 — Enemy deck realism: colour-loyal permanent removal · deeper difficulty variation · Grakk ramp/ability permanents · balance  *(bullets 9 · 25 · 26)*

**Bullet 9 — every deck needs colour-loyal artifact/enchant (permanent) removal.** *Grounded:* the `naturalize` driver is **fully wired but unused** — `applyTarget('naturalize')` (~1258) destroys a player artifact/enchant honoring hexproof/shroud/prot+ward; AI gate `enemyInstantWouldDo` case `'naturalize'` (~2656); value `bestTargetThreat` (~2677). But the only `naturalize` FX is a `_`-prefixed **test** card (`_tnaturalize` "Cleansing Bloom" G, ~855); `bounce` likewise (`_tbounce` U ~852). **No warden pool** (Grakk R ~861 · Murglax B ~867 · Vael RB ~877) has any — `voidcall` hits only planeswalkers. *Change:* add ≥1 colour-loyal `naturalize` (or `bounce`) FX to **each** pool (R for Grakk, B for Murglax, R/B for Vael), reusing the existing driver/gate/value — data only. Colour-pie caveat: mono-black Murglax reliably destroying artifacts is a stretch (give it a life-cost black variant, or accept the combined artifact-or-enchant target).

**Bullet 25 — deeper per-difficulty deck variation.** *Grounded:* difficulty swaps **exist** (`buildDeck` ~987–993 applies `room.swaps[S.diff]` cut/add) but are small (~2–5 cards; easy mostly swaps finishers for lands = flood, brutal trims a 1-drop for one gas/counter); there's no `swaps.standard` (standard = base). *Change:* broaden each warden's `swaps` (pure data in `DUNGEON`) so **card identity** — not just land count — differs across easy/standard/brutal (easy cuts more removal/finishers for vanilla bodies; brutal raises removal density + a 2nd finisher/counter/naturalize). Keep total size + land ratio sane (the ~991 cut/add loop already guards under/overflow).

**Bullet 26 — realistic decks (ramp/ability permanents, some ability creatures, balanced).** *Grounded:* **Murglax + Vael already satisfy this** — obol/altar mana rocks (`run:['ramp',N]` ~795/796), the bloodpact "Pit's Tithe" aristocrats engine, `raisedead`/`reclaim` graveyard package, deathtouch/lifelink/menace bodies. **GRAKK is the gap** — zero artifacts, no ramp, and its only non-creature permanents (auras/warbanner/furnace) **punish the player** rather than ramp/grant abilities. *Change:* give Grakk ≥1 red artifact / ability-granting enchant fitting aggro — a ritual burst (`run:['ritual',N]` ~1222) or a Treasure-maker (**note: there is NO `applyRun` `'treasure'` case yet** — that path needs a small new op) or a static goblin anthem. All content lands via the existing `_enemyCardToPerm` (~2473) → real `S.enemyArtifacts`/`S.enemyEnchants` permanents. **Balance pass** across difficulties (heaviest cards: `undying` 7/7 ~844, `inferno`, the finishers) so nothing trivially wins on standard/brutal; keep Grakk's fast curve.

**Reconciliation:** the naturalize mechanic + AI were built in **P44.4** (reserved for future colours — this promotes it into R/B Level-I content); ramp/ritual/graveyard/aristocrats infra is **P35**; enemy artifacts/enchants are real permanents via `_enemyCardToPerm` (**P13.2/13.3/14.3**). The three bullets interlock: new removal (9) is natural brutal-swap fodder (25) and "realistic content" (26). Grakk now also fields the Siege (P48.3, ~860) so it isn't wholly permanent-less. **Open Qs:** artifact-only vs combined naturalize target for Murglax; how large the difficulty spread + is standard the untouched base; **what Grakk's red artifact should DO** (Treasure reads off-archetype and needs a new op; an anthem risks power creep; a haste-granter is redundant); **no defined balance metric** exists to tune "not overpowered" against — what win-rate / turns-to-lethal target?

**Verify:** jsdom — for each room `buildDeck` yields a `naturalize`/`bounce` key whose colour ⊆ room colours, and with a player permanent present `enemyInstantWouldDo` is true and `applyTarget` destroys it; build each warden under easy/standard/brutal and assert the differing non-land multiset + sane land ratio (optionally monotonic summed `threatScore` easy<std<brutal); cast Grakk's new artifact/enchant → it lands on `S.enemyArtifacts`/`S.enemyEnchants` and its effect fires.

## P49.9 — Enemy coloured/colourless mana + London mulligan penalty  *(bullets 16 · 24)* — **#16 reverses the P44 default**

**Bullet 16 — real coloured/colourless costs + exact-mana casting + AI.** *Grounded:* enemy mana is a **single generic integer** — `S.bossMana`/`Max`/`Frozen`/`Lands`/`Mod`, one accessor `usableMana()` (~1174). FX cards carry `cost:<number>` (total CMC) + `color:"R"/"B"/"RB"`, but colour is used **only** for display dots and protection/targeting, **never** for affordability. Every cast site checks `fx.cost<=usableMana()` (~2318/2337/1522/2696/1245/treasure ~3375) and spends `S.bossMana-=fx.cost` (~2344/1545/2697/1042); playing a land **discards** its colour (`S.bossLands++;S.bossMana++`). **SPEC P44's open-Q default is explicitly "NO real coloured costs"** — this bullet reverses it. *Change (large):* (1) **pip data** — either hand-author a per-card breakdown (`mc:"1RB"`, ~200 cards, correct) or auto-derive a heuristic from `cost`+`color` (mono = 1 colour pip + generic; RB = 1R+1B+generic — far less work, must be documented); (2) a **per-colour pool** (`S.bossPool={R,B,C}` or a parallel avail alongside the scalar) + init (~983) + save-migration (~3075/3088); (3) record land/ramp colour at the 3 play sites (~1041/2202/1221); (4) a single **`canAfford(fx)`** accessor replacing every `cost<=usableMana()`; (5) a colour-aware **`payCost(fx)`** replacing every `S.bossMana-=`; (6) **AI** — `vaelMain`'s greedy value loop filters on `canAfford`, and the land-from-hand pick chooses the colour that unlocks the most value; (7) per-colour pip **display** (~1703–1709). **Only the Vael R/B fight is meaningfully affected** — Grakk (mono-R) and Murglax (mono-B) satisfy coloured costs trivially.

**Bullet 24 — London mulligan penalty (−1 card per mulligan).** *Grounded:* `dealOpeningHand` (~1000–1014, P17.2/P44.5) is a **free** mulligan — it always redraws a full 7; `att` = number of mulligans taken. *Change:* on the KEEP branch (~1010), if `att>0` bottom `att` cards (worst first: excess lands beyond the window, then top-end) so the kept hand is **7−att** cards; the CAP branch (~1013) also keeps 7, so store a `bestAtt` (record it wherever `best` is updated) and trim by it there too. Update the log lines to state the final hand size. Contained to `dealOpeningHand`.

**Reconciliation:** #16 extends P17.1 lands-only (coloured lands are natural) and P5.1's single-choke `usableMana` (add a **sibling** `canAfford`/`payCost`, don't overload); P35.2 dorks/rituals/Treasures each need a colour (or colourless C); complements P44.4 colour-pie flavour. #24 modifies P44.5 mulligan (the window/curve scoring stays; only the kept-hand **size** changes) and makes the mulligan a real cost under P17.1. **Open Qs:** hand-authored pips vs heuristic; gate **all** costs (huge, near-no-op for the two mono decks) or **only** the Vael deck; what colour do colourless sources produce; is freeze per-colour or a generic cap; cap the mulligan penalty (a brutal deck could self-mill to 4–5 cards)?

**Verify:** jsdom — at the Vael room, `S.hand=[Mountain, a color:"B" spell]`, drive `vaelMain`, assert the B spell is **not** cast until a black source is present; give exactly 1R+1B + an RB cost-2 card → casts, then 2R only → doesn't; save→load preserves the colour pool (no NaN); grep for remaining bare `S.bossMana-=`/`cost<=usableMana()`. Mulligan: stub RNG to force N mulligans → `S.hand.length===7-N`, `S.lib` grew by N, the pre/post card multiset is conserved; the CAP path trims by its best hand's attempt count.

## P49.10 — Combat/PW UX: combat popup minimize/restore · PW abilities off-stack + popup · battles side-selectable · resurgent timing  *(bullets 19 · 22 · 11 · 10)* — **#22 reverses P38**

**Bullet 19 — minimize/restore the combat popup (turn stays blocked).** *Grounded:* combat is the body-level `#resolver` `.resolver` modal (~571–579, no backdrop-dismiss); **every "can't advance" guard tests `S.combat` non-null, NOT modal visibility** — `advancePhase` (~2175), `flowClick` (~2172), `backPhase` (~2186), and the disabled `#dmBtn`/`#backBtn` (~2404–2405). *Change:* add a **CSS-only** minimize — a "reduce" button that adds `.resolver.min` (mirror the `.stackpop` bottom-sheet pattern ~118–136: drop the backdrop, `pointer-events:none` container / `:auto` modal, dock to a corner) + a `.stackfab`-style re-expand pill (~133). Because the guards key on `S.combat`, minimizing leaves the turn fully blocked **automatically** — no gating change. Optionally persist `S.combat._min` so a mid-combat `render()` (~1724) doesn't force it open. Minimizing must NOT resolve/cancel combat.

**Bullet 22 — PW abilities bypass the stack + announce via popup.** *Grounded:* **enemy walker abilities currently GO through the stack** — `enemyWalkerAct` (~2237) applies the loyalty delta then `mkPlay(...);S.plays.push(p)` (~2239) → the effect resolves later in `resolvePlay` (~2521); `enemyWalkersAct` (~2244) then `showPlays()`. Abilities are Ash the Guardian's (~872–875). *Change:* resolve the ability **directly inline** (mirror resolvePlay's generic branch: `applyRun(ab.run)` / `applyTarget(ab.target,…)` / `selfLoss`) and surface an **announcement popup** (reuse `showToast` ~1684 or a small dedicated modal — the bullet says "popup"); drop the `showPlays()`/empty-stack gating in `enemyWalkersAct`. **Keep** the loyalty delta, `enemyWalkerDies` (~2246), the once-per-turn `_actedTurn` gate (~2238), and the ult 8/8 Avatar spawn. **REVERSES P38** (~2239/2241 deliberately routes abilities through the stack "so the player can respond"). **Player walkers have NO ability engine** (`addW` ~1922 creates no `wplus/wminus/wult`; `walkerDrawer` ~1822 has no activation) — so "player PW abilities via popup" means first **building** that engine; confirm scope.

**Bullet 11 — battles/sieges selectable player-owned or enemy-owned.** *Grounded:* `addBattle(side)` (~1892) and the `side:'you'|'boss'` field **already exist**; only the UI hardcodes `addBattle('you')` (~553), and `renderBattles` (~1914) shows a display-only side badge. *Change:* (1) add-time side selector at ~553 (two buttons or a `<select>` feeding `addBattle(sel.value)`); (2) a per-row side toggle in `renderBattles` calling `setBattle(id,'side',…)`. A user-added boss battle lacks `_siege`, so it won't collide with `fieldBossSiege`'s one-siege-per-room guard (~1903); `battleDefeat` already branches on `side==='boss'`. Expose `tick` if an added boss siege should heal like Grakk's auto one.

**Bullet 10 — resurgent timing (confirm/refine).** *Grounded:* **already correct for resolved creatures** — the Resurgence 1/1 spawn in `vaelMain` (~2346) fires only `which===1` (end of main-1) and only when `S.tokens.length===0` (empty enemy board) + no commander in play/pending. *Change:* mostly confirmation. One optional refinement: the guard reads `S.tokens` (resolved) but not an enemy creature spell still **pending on `S.plays`** this main phase → add `&& !S.plays.some(p=>p.status==='pending'&&(p.type==='creature'||p._enemyCmd))` or relocate the check after the enemy stack resolves.

**Reconciliation:** #19 mirrors the P45.6/P1.10 stack-sheet (`.stackpop`/`.stackfab`) rather than inventing a pattern; the P32/P44.1 trick window renders inside `#combatBox` and is unaffected. #22 reverses P38 — update the `#pwPanel`/tutorial copy implying stack interaction. #11 touches P7.7/P48.3 (the guard keys off `_siege` so an added boss battle coexists with Grakk's auto-siege). #10 is self-contained (P38/P5.2). **Open Qs:** combat minimize in both directions + auto-restore when a trick window opens; PW-popup scope (enemy-only vs build the player engine) + blocking vs non-blocking toast; should a user-added boss siege heal by default; does resurgent's "no creatures" test include pending-on-stack creatures?

**Verify:** jsdom — `openCombat` → `.show`; reduce → `.resolver.min` with `S.combat` still set; `advancePhase`/`flowClick` no-op + `#dmBtn.disabled`; render doesn't force-expand; `approveCombat`/`cancelCombat` clear `S.combat`. PW: force each Ash branch, `enemyWalkerAct` → `S.plays.length` unchanged, effect applied (life loss / greatest-power destroy / 8/8 spawn), popup invoked, loyalty changed. Battles: `addBattle('boss')` → `side==='boss'`, `battleDefeat` hits the boss; `setBattle(id,'side','you')` flips to the reward branch; `fieldBossSiege` still adds one `_siege`. Resurgent: empty board → one 1/1 + a "Resurgence" log; a resolved creature present → no spawn; (refinement) a pending creature → no spawn.

## P49.11 — Vael / Ash the Guardian: reborn at 1 HP · +1 Ash Soldier · −3 graveyard reanimator  *(bullets 23 · 27 · 28)*

**Bullet 23 — Vael reborn at 1 HP.** *Grounded:* the reborn ladder is on the room def (~871) `reborn:{easy:10,standard:15,brutal:20}`, consumed in `bossDown` (~2158) to set `S.boss.life` (with a `Math.max(1,…)` floor), alongside a Pyre Revenant 4/3 spawn, gated by `S.phase2done`. *Change:* set `reborn:{easy:1,standard:1,brutal:1}`. Update the two lore strings that echo the values (`~2874` "reborn 10/15/20…", the P10 comment `~2158`). Keep the Pyre Revenant spawn + `phase2done` gating; the ENRAGED log auto-reads `${S.boss.life}` → "rises ENRAGED at 1 life".

**Bullet 27 — Ash the Guardian +1 → "Create a 1/1 Ash Soldier token with haste".** *Grounded:* the current +1 (~873) is `Ember Whisper` `selfLoss:2` — despite the "You lose 2 life" wording it's applied to the **player** (`S.youLife-=2` at resolve ~2521). *Change:* replace with `plus:{sign:1,n:"…",text:"Create a 1/1 Ash Soldier token with haste.",run:["spawn","Ash Soldier",1,1,["haste"],"R"],color:"R"}` (drop `selfLoss`). Reuses `applyRun` case `'spawn'` (~1220, sets `sick:!haste`→untapped-ready) — the **same** mechanism as the ult's 8/8 Avatar (~875); `enemyWalkerAct` already forwards `run:ab.run`, so no activation-path change. Update the mirrored lore (~2873).

**Bullet 28 — Ash the Guardian −3 → graveyard exile-X + free reanimate ≤X.** *Grounded:* the current −3 (~874) is `Grasping Ash` `target:"destroyMaxPow"`. A reanimator primitive already exists: `applyRun` case `'reanimate'` (~1224–1226) filters `S.gy` for creature FX, picks by body value, splices it out, builds a real permanent via `enemyBodyFromFX` (~2483) → `S.tokens`; mana value = the FX's `cost`. *Change:* make the −3 a graveyard reanimator — `text:"Exile X creatures from Vael's graveyard, then cast a creature with mana value X or less from his graveyard without paying its mana cost.", run:["ashReanimate"]` (drop `target`). Add a `case "ashReanimate":` beside `'reanimate'` (~1219): X = count of creature-typed cards in `S.gy`, move those X → `S.exile`, then reanimate the best creature with `FX[key].cost<=X` via `enemyBodyFromFX` → `S.tokens`. Update the mirrored lore (~2873) and the "−3 is removal" tip (~2877). **Fix the AI:** `pickWalkerAbility` (~2221–2236) currently treats the −3 as **defensive removal** (snipe the player's bomb) — it no longer does that; retune the heuristic and add an empty-graveyard fizzle guard so the AI doesn't waste 3 loyalty on a no-op.

**Reconciliation:** P10 reborn ladder; P38.3 rename/abilities; P38 walker-play model; P35.4 reanimation infra. Villain stays "Vael, the Ember Tyrant" (~871), walker stays "Ash the Guardian" (~872) — no P48.3 name conflict. **Open Qs:** does "exile X" = ALL graveyard creatures set X (assumed) or a chosen subset (and are non-creatures counted)?; does the free-cast re-trigger ETB or enter body-only (assumed body-only, matching `enemyBodyFromFX`)?; is the Pyre Revenant token also weakened (assume no — the bullet is HP-only)?; +1 token name/colour casing + keep the "Ember Whisper" name?

**Verify:** jsdom — at the Vael room, for each difficulty set `phase2done=false;boss.life=0`, `bossDown()` → `boss.life===1`, `phase2done===true`, a "Pyre Revenant" on `S.tokens`; a 2nd reduction does NOT revive. +1: force `pickWalkerAbility`→plus, `enemyWalkerAct`+resolve → `S.tokens` gains `{name:'Ash Soldier',p:1,t:1,kw⊇['haste'],sick:false}`, `S.youLife` unchanged, loyalty +1. −3: seed `S.gy` with creature stubs (some `cost<=X`, some `>`), force the minus, resolve → X creatures moved `S.gy`→`S.exile`, one reanimated body with `cost<=X` on `S.tokens`, loyalty −3; empty-gy case is a clean no-op. Grep the built file for "10/15/20".

## P49.12 — Items & cards: enemy top/bottom-N full info · art on saved cards  *(bullets 14 · 17; bullet 13 RESOLVED — no change)*

**Bullet 13 — unused active items persist until consumed — RESOLVED 2026-07-07, OUT OF SCOPE (no work).** The user confirmed the intent was **only unused active (consumable) items**, and that is **already the behavior**: within a run `S.inv` is never cleared; at run end `saveUnusedItems` (~3027) stashes `kind==="consumable"` to `bucket().stash` and `applyStashItems` (~3028) re-injects them next battle, so an unused consumable survives a loss and carries into the next level, while a used one (spliced by `useBoon` ~2154, P21) does not return. **Passives/reminders correctly expire on descend** (P14.5 stays — do NOT widen the `kind==="consumable"` filter; that would re-open the P42.1 immortal-passives loop). Nothing to build; keep a regression assert if convenient.

**Bullet 14 — enemy top/bottom-N (and hand) reveal must show full card info.** *Grounded:* the P31 expandable-row helper `fxItem(c,actions)` (~1091, chevron→`dtToggleCard` ~1048, `fxExpandBody` ~1076 with P/T·keywords·cost·colours·oracle·flavour) is used by the play/scry/browse/discard sub-views — but the `_dt.reveal` block (~1137, output of `dtLook('top'/'bottom')` ~1143 and `dtReveal()` hand ~1144) uses a **bespoke** name+cost row with **no chevron/body**, so a top/bottom-N look shows no stats/effects (matching the complaint). *Change:* swap the reveal row to `fxItem(c, acts)` (pass the per-zone move buttons as `actions`); add a `dtExpandAllBtn` to the section header. S.lib/S.hand cards carry a stable `.id` (browse already proves `fxItem` works on them). Also aligns with the tutorial copy (~2991) that already claims rows expand.

**Bullet 17 — add art to already-saved homebrew cards (feasibility → YES).** *Grounded:* homebrew cards store **no** art (`readCastForm` ~3241 builds cfg with no image/art; `castFormHTML` ~2597 has no art input). `cardArt(o)` (~1644) reads `o.art||o.image`, **http(s)-only**, renders a `.cardart` strip used everywhere incl. `renderLibrary` (~3390). Scryfall imports **do** persist art (`buildImportedCard` ~3460 → `cfg.image`/`cfg.art`); the `SCRY` layer (`sfFetch`/`sfSearch`/`sfCollection` ~3471–3474) works offline-after-save. *Change (pick one, increasing cost):* **(a) simplest** — add an "Art URL" input to `castFormHTML`; `readCastForm` sets `cfg.art` when it matches `^https?://` (+ `loadCastFromLibrary` ~2586 pre-fill); renders unchanged, works offline. **(b)** a "🎨 Fetch art from Scryfall" button reusing `sfCollection([name])` → writes `cfg.art` (online-only, only when the name matches a real card). **(c)** upload/paste a `data:` URI — needs `cardArt` to accept `data:` **and** risks the localStorage quota (base64 in the profile JSON). **Recommend (a), optionally (b).** *Adjacent bug to fix in the same pass:* editing an **imported** (art-bearing) card **strips its art** — `saveCardEdits`→`readCastForm` rebuilds cfg without image/art; carrying the field through fixes it.
...
## P50.9 — Smarter enemy strategy (mana + attacks + blocks)  *(user 2026-07-08, item 6)* — ✅ **DONE** (v67)

**Shipped (`build/p50-9-final`, via an isolated git worktree to avoid the parallel session's live edits).** Three targeted heuristics, all gated on `enemyLuck()>=0` (easy stays legacy), addressing the user's "way smarter, strategize attacks/blocks/mana" directive on top of the already-sophisticated P34 AI (value-based casting, hold-up reserve, `gangToKill`, survival-first blocking): **(A) attacks** — `profitableBlock` (in `vaelAttackers`) now also holds a bomb back if the player can GANG-kill it at a favourable trade (`gangToKill` + a `resolveAttack` value check: the player's lost `threatScore` < the attacker's), so the enemy won't alpha-strike its best creature into a gang-block; LETHAL PUSH / wide / evasive still override. **(B) blocks** — the non-lethal chump no longer throws away a valuable body when the boss is healthy; it chumps only if the hit is genuinely dangerous (boss would drop to ≤25% max / ≤6) or the body is cheap (≤½ the threat's value). Survival-first still catches true lethal. **(C) mana** — `enemyHoldUpReserve` returns 0 (develop) when clearly behind on board (your creatures outvalue its by >1.5×+4) instead of sandbagging interaction mana. Single-agent adversarial review → NO CONFIRMED FINDINGS (verified non-mutating, no crash, gating correct, lethal-safe). Driver `tests/p50-9-ai.test.js` (13 asserts). npm test 47/47. Original spec below.
...
- **Level-I counterspell distribution (P44.4):** the colour pie makes R/B counter-magic rare and conditional. *Default:* Grakk 0 · Murglax 0 · Vael exactly ONE punisher-style conditional counter (Molten Influence / Dash Hopes family; easy's swap removes it); NO hard counters anywhere at Level I. Alternative if even one feels off in play: zero everywhere, and the mechanic ships engine+driver-only until a blue enemy exists.
- **Library-only op build depth (P44.4):** `bounce` builds now (cheap over the existing mover). Do `fight` + player-artifact/enchant removal also build now, or stay specced-only until the first G/W level? *Default:* build them at P44.4 time only if the shared targeting keeps each small; otherwise leave them specced one-`applyRun`-case-away — either way driver test-cards only, never Level I pool cards.
- **License (P46.1):** add one? *Default:* none (private repo, all-rights-reserved) until the user says otherwise.

### Phase 49 decisions (2026-07-07 fix batch — confirm before building the flagged items)

- **Enemy coloured mana scope (P49.9 / bullet 16):** this **reverses** the standing P44 default ("costs stay generic numbers"). Two forks: (a) **pip source** — hand-author per-card coloured pips (~200 cards, correct) vs an auto-derived heuristic (mono = 1 colour pip + generic; RB = 1R+1B+generic; fast, less exact); (b) **breadth** — gate *all* enemy costs (huge data pass, near-no-op for the two mono decks) vs *only* the two-colour Vael deck (much smaller, where it actually bites). *Recommended default:* heuristic pips, Vael-only, colourless sources produce generic C, freeze stays a generic total cap. Confirm.
- **Automate enemy instants (P49.3 / bullets 20·21):** **reverses** P4.2/P4.4 (player approves/edits the enemy's response). *Default:* auto-cast the best worthy response, still counterable by the player; keep the combat-proposal path intact; retire the instant-proposal UI. Confirm the player keeps no approve step, and whether proactive instant-speed *windows* also auto-fire.
- **PW abilities off the stack (P49.10 / bullet 22):** **reverses** P38 (abilities resolve as stack items so the player can respond). *Default:* **enemy** walkers only — resolve directly + a non-blocking announcement toast. Player-walker parity would require first *building* a player PW ability engine (none exists) — build it too, or defer? 
- **Commander tax model (P49.2 / bullet 15):** ~~flat +2 per cast vs the current doubling~~ **DECIDED by the user (2026-07-07) — model it exactly like real Commander:** +2 for each time the commander has **previously been cast from the command zone** this game (flat, cumulative). Increment **only** on a command-zone cast; a cast **from hand is untaxed and does not increment**; death/bounce/counter don't raise it (they only return it to the command zone at the current tax); "Deploy direct" counts as a command-zone cast; tax resets per dungeon. Same rule for the enemy commander. See P49.2 15B.
- **Item persistence breadth (P49.12 / bullet 13):** ~~does "active items" also include passives?~~ **DECIDED by the user (2026-07-07) — consumables only, no rule change.** Unused *consumables* already persist across losses & levels (P15.3 stash); **passives/reminders correctly expire on descend** (P14.5 stays — do not widen the `saveUnusedItems` filter). Bullet 13 needs no work; dropped from the build scope.
- **Italian localization scope (P49.6 / bullet 7):** *Default:* language is **global** on `DB.lang` (one menu flag, default EN); a `location.reload()` on switch is acceptable for the static-at-load chrome (autosave makes it lossless); the ~305-line **Dungeon Log** narration **is in scope** (translate it too); **MTG rules keywords stay English** (commander/planeswalker/deathtouch…). Flip any of these if you'd rather narrow the work (excluding the log roughly halves it).
- **Wheel + gold co-tune (P49.5 / bullets 6·18):** `wheelBase()` reads `goldReward`, so retuning gold rescales every wheel payout — they must be tuned together. *Needs a target:* intended gold-per-clear (today a std clear banks ~66g against a 42g-max store), and whether the finale room should grant a flat base at all (today it's wheel-only). Lever = income (`goldReward`) vs prices (P42.4 already tuned prices).
...
# PHASE 51 — Fix batch 2026-07-10 ✅ ALL 11 built & pushed (P51.7 v69 · P51.10 v70 · P51.11 v71 · P51.2 v72 · P51.8 v73 · P51.5 v74 · P51.1 v75 · P51.3 v76 · P51.9 v77 · P51.6 v78 · P51.4 v79 · P51.6-fix v80) — origin/main @ `fe658f7`
