...
| &nbsp;&nbsp;P49.6 Italian localization (cutscene/lore/UI/tutorial/info/abilities/shop/menus/library/settings, keep NAMES) + main-menu flag selector (default EN) | 🟢 **7a INFRA + core chrome DONE (v89)** — `DB.lang` (default en) · `t()`/`L()` dictionary helpers (graceful EN fallback) · 🌐 menu flag + Settings language row · `applyLang()` static-chrome tree-walker · `setLang()` save+reload. Translated: menu · settings · tabs · difficulty/saves · mode-screen labels · phase/turn-flow labels · INFO titles + short bodies · arrival narrative · cutscene buttons. **EN unchanged, IT boots + full turn cycle 0 errors; proper nouns + MTG keywords stay EN.** Extensible dictionary — remaining prose (full tutorial, warden lore, long info bodies, shop text, ~305 log lines) falls back to EN, translatable by adding dictionary entries. |
