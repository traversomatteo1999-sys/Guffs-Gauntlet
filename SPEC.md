...
| **Phase 13 — Symmetric enemy board: full enemy-card editing + first-class artifacts/enchantments + deeper automation** | 🔨 **IN PROGRESS** — P13.1 done; P13.2 automation + P13.3 first-class artifacts/enchants remain. See Phase 13 below |
...
| **Phase 18 — Combat correctness: menace enforcement + keyword automation audit** | ⬜ **PLANNED** — **diagnose first**, then verify/harden the P12.2 combat engine: a menace creature is blockable **only** by 2+ and counts as blocked **only** when ≥2 blockers are assigned (fewer = invalid, treated as unblocked/refused) — identically for enemy and player creatures · deathtouch · lifelink · trample · first/double strike · vigilance · indestructible · protection all auto-resolve correctly on resolution, in **both** combat directions. See Phase 18 below |
| &nbsp;&nbsp;P18.0 Diagnostic FIRST — reproduce menace/keyword scenarios both directions, record actual behavior, scope the fixes | ⬜ planned |
| &nbsp;&nbsp;P18.1 Menace: blockable only by 2+, counts as blocked only when ≥2 assigned (enemy + player, both directions) | ⬜ planned |
...
> **⚠ START HERE — establish ground truth before changing anything (P18.0).** The user observed that the UI *let them assign a single blocker to an enemy menace creature* and is not sure the engine actually treated it as an illegal/under-block — and reasonably suspects the rule may be enforced for the **player's** menace creatures but not the **enemy's** (the two must behave identically). On a code read the enforcement *looks* symmetric — `approveCombat`'s refuse-loop (~1168) iterates `attackers` in **both** directions and `minBlockers` reads `kw(c,'menace')` off any object — so the single-blocker UI assignment is expected to be **refused at Approve** (with a `predictCombat` ⚠ first), not silently resolved. **But do not assume.** The first deliverable of this phase is a diagnostic (P18.0) that *reproduces the user's exact scenario in both directions* and records what actually happens; only then do P18.1/P18.2 fix whatever the diagnostic proves is broken. If the diagnostic shows it already works, the tasks collapse to "add regression coverage + close the UI gap that let a single blocker be assigned in the first place."
...
3. **`minBlockers` symmetry:** assert `minBlockers(enemyMenaceToken)===2` and `minBlockers(playerMenaceCreature)===2` and `minBlockers(S.cmd)` when the commander has menace — confirm the helper reads `kw` off every object shape (`S.tokens`, `S.cmd`, `S.my.creatures`).
...
**Deliverable:** a short findings note appended to this phase (✅ works / ❌ broken / ⚠ UI-only gap, per scenario) that **scopes P18.1/P18.2**. If everything resolves correctly, P18.1 narrows to "add the regression tests above + optionally tighten the UI so a single blocker can't be *assigned* to a menace attacker (immediate inline feedback instead of an Approve-time refusal)." If the enemy direction is genuinely unenforced, P18.1 fixes that specific gap.
...
**How (driven by the P18.0 findings — fix only what the diagnostic proves is broken):**
1. **Symmetry is the priority:** whatever P18.0 found, ensure the min-block rule fires identically for `dir==='you'` and `dir==='vael'`. `approveCombat` (~1168) rejects a `1..min-1` block by iterating `attackers` (direction-agnostic by construction) — verify this actually triggers for an **enemy** menace attacker blocked by 1, not just a player one; if a direction is skipping it, fix the gap. `aiBlocks` (~1084) already won't *propose* `<min`; confirm `predictCombat` (~1162) warns before approve in both directions.
2. **Close the UI gap (user-visible fix):** since the user *saw* a single blocker get assigned, give immediate feedback at assign time — either block the assignment in `combatAdd` (~1152) when it would leave a menace attacker under-blocked with no path to reach min, or surface the ⚠ inline on the block row — so the player isn't surprised at Approve. (Keep the Approve-time refusal as the hard guard.)
